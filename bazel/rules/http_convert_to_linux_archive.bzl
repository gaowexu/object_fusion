""""Rules for downloading windows generated archives over HTTP.

### Setup

To use these rule, load them in your `WORKSPACE` file as follows:

```python
load("@//bazel/rules:http_convert_to_linux_archive.bzl", "http_convert_to_linux_archive")
```

This rule is a copy of the native http_archive rule.
The only difference is, after extracting the code on linux folders will be created according to filenames.
"""
...
def _http_archive_convert_to_linux_impl(ctx):
    ...
    if ctx.os.name.lower().find("windows") == -1:
        ...
        cmd_move_dot = """for file in .\\\\*; do target="${file//.\\\\/}"; mv -v "$file" "$target"; done"""
        st = ctx.execute([bash_exe, "-c", cmd_move_dot])
        if st.return_code:
            fail("Error applying patch command %s:\n%s%s" % (cmd_move_dot, st.stdout, st.stderr))

        cmd_create_dirs = """for file in *\\\\*; do target="${file//\\\\//}"; mkdir -p "${target%/*}"; mv -v "$file" "$target"; done"""
        st = ctx.execute([bash_exe, "-c", cmd_create_dirs])
        if st.return_code:
            fail("Error applying patch command %s:\n%s%s" % (cmd_create_dirs, st.stdout, st.stderr))
...
http_convert_to_linux_archive = repository_rule(
    implementation = _http_archive_convert_to_linux_impl,
    attrs = _http_archive_attrs,
    doc =
        """Downloads a Bazel repository as a compressed archive file, decompresses it,
and makes its targets available for binding. Archives created in Windows will contain '/'.
After extracting in Linux this will expand to the filename. This rule recreates the file structure
in Linux by parsing the filenames of the extracted files.
